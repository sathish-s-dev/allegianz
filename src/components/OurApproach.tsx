...
const OurApproach = () => {
  const approaches = [
    {
      ...
      description: "We begin with a thorough assessment of your business, market position, and challenges. This helps us understand your needs and define a clear path forward."
    },
    {
  ...
  return (
    <section id="approach" className="bg-[rgba(246,246,246,1)] max-w-full w-[1514px]">
      <div className="bg-[rgba(255,255,255,0)] flex min-h-[307px] w-full flex-col pt-[38px] pb-[67px] px-[83px] max-md:px-5">
        <div className="w-[1280px] max-w-full">
          <div className="w-[528px] max-w-full text-[84px] font-medium whitespace-nowrap max-md:text-[40px]">
            <div className="flex w-full justify-center flex-wrap max-md:text-[40px]">
              <h2 className="text-[rgba(22,22,22,1)] leading-none tracking-[-1.76px] w-[156px] max-md:text-[40px]">
                Our
              </h2>
      ...
      <div className="w-full">
        <div className="flex w-full items-center flex-wrap">
          {approaches.map((approach, index) => (
            <article key={index} className="bg-white border flex min-w-60 min-h-[632px] flex-col overflow-hidden items-stretch grow shrink w-[305px] pl-6 pr-[22px] pt-6 pb-[215px] rounded-[0px_10px_0px_0px] border-[rgba(203,214,255,1)] border-solid max-md:pb-[100px] max-md:px-5">
              <img
                src={approach.icon}
              ...
              <div className="w-full mt-[17px]">
                <h3 className="text-[rgba(15,15,15,1)] text-[39px] font-semibold leading-[47px]">
                  {approach.title.includes('&') ? (
                    <>
                      {approach.title.split('&')[0].trim()} <br />& {approach.title.split('&')[1].trim()}
                    </>
                  ) : (
                    approach.title
                  )}
